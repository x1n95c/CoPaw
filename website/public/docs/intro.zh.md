...
1. **在聊天软件里对话**
   在钉钉、飞书、微信、Discord、Telegram等app里发消息，CoPaw 在同一 app 内回复，
...
- **控制台** — CoPaw 内置的 Web 管理界面，可以在控制台中对话、配置频道、管理技能、
...
- **MCP 和工具** — MCP（Model Context Protocol）是一种标准协议，允许接入外部
  工具服务器扩展能力。工具是 CoPaw 内置的基础能力（如读写文件、执行命令、
  浏览器等）。详见 [MCP和工具](./mcp) 。
- **智能体/工作区** — 从 v0.1.0 开始，CoPaw 支持多智能体，允许运行多个独立的
  AI 智能体。每个智能体拥有独立的工作区、配置、记忆、技能和对话历史，智能体之间
  还可以通过协作技能互相通信、共同完成复杂任务。详见 [多智能体](./multi-agent)。
- **安全机制** — CoPaw 提供多层安全防护，包括工具防护（拦截危险命令参数）、
...
2. **[控制台](./console)** — 服务启动后，在浏览器中打开控制台（`http://127.0.0.1:8088/`），
   **这是配置与使用 CoPaw 的中心枢纽**。先在控制台体验对话、配置模型，有助于理解
   CoPaw 的工作方式。
3. **[模型](./models)** — 配置云端 LLM 提供商的 API Key，或下载本地模型。这是使用
   CoPaw 的**必要前提**。
4. **按需配置与使用**：
   - [频道配置](./channels) — 接入钉钉 / 飞书 / 微信 / Discord / Telegram 等，在对应 app 里与 CoPaw 对话；
