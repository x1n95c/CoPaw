...
class ACPToolCallParser:
    ...
    _STATE_KEYS = ("title", "kind", "status", "rawInput", "rawOutput", "content")

    def __init__(self, agent_name: str, tool_parse_mode: str, tool_calls: dict[str, dict[str, Any]]):
        self.agent_name = agent_name
        self.tool_parse_mode = tool_parse_mode
        self.tool_calls = tool_calls

    def handle_tool_call_created(self, update: dict[str, Any]) -> dict[str, Any] | None:
        return self._handle_tool_call(update, created=True)

    def handle_tool_call_updated(self, update: dict[str, Any]) -> dict[str, Any] | None:
        return self._handle_tool_call(update)

    def _handle_tool_call(self, update: dict[str, Any], *, created: bool = False) -> dict[str, Any] | None:
        ...
        if created and self.tool_parse_mode == "update_detail":
            return None
        return self._tool_message_from_state(state, event_type=self._event_type(state, created=created))
    ...
    def _event_type(self, state: dict[str, Any], *, created: bool) -> str:
        ...
        }.get(self._string_value(state.get("status")) or "pending", "tool_start")

    def _merge_update_into_state(self, state: dict[str, Any], update: dict[str, Any]) -> None:
    ...
    def _tool_message_from_state(self, state: dict[str, Any], *, event_type: str) -> dict[str, Any]:
        ...
        message["detail"] = self._extract_detail(state, kind=message["kind"]) or title
    ...
    def _extract_detail(self, state: dict[str, Any], *, kind: str) -> str | None:
        if self.tool_parse_mode == "call_title":
            return self._string_value(state.get("title"))
        if kind == "execute":
            return self._extract_command(state)
        if kind == "read":
            return self._raw_input_first(state, "file_path", "filePath", "path")
        if kind == "search":
            return self._raw_input_first(
                state,
                "path",
                "pattern",
                fallback=self._extract_target(state) if self.tool_parse_mode == "call_detail" else None,
            )
        return None
    ...
    def _raw_input_first(self, state: dict[str, Any], *keys: str, fallback: str | None = None) -> str | None:
        for key in keys:
